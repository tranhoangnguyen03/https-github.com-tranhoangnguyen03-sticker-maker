...
const App: React.FC = () => {
  ...
  const [apiKey, setApiKey] = useState(() => {
    ...
    // 2. Check local storage (runtime persistence)
    if (typeof localStorage !== 'undefined') {
      return localStorage.getItem('sticker_maker_api_key') || '';
    }
    return '';
  });
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex flex-col items-center">
        ...
        {showLanding ? (
            ...
            <div className="w-full max-w-4xl flex flex-col gap-8 animate-in fade-in zoom-in duration-300">
                {/* API Key Section */}
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <Key className={`w-5 h-5 ${hasKey ? 'text-green-500' : 'text-amber-500'}`} />
